...
public class Run {
    public static void main(String[] args) {
        ...
        try {
            ...
            TypedQuery<LMember> query = entityManager.createQuery("SELECT m from LMember m where m.age > :age", LMember.class);
            query.setParameter("age", 20);

            List<LMember> members = query.getResultList();
            ...
            TypedQuery<LMember> newQuery = entityManager.createQuery("SELECT m from LMember m where m.age > ?1", LMember.class);
            newQuery.setParameter(1, 40);

            List<LMember> newMembers = newQuery.getResultList();

            for (LMember m : newMembers)
                System.out.println(m.getName() + ", " + m.getAge());
